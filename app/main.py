...
async def advice(request: Request):
    ...
    response = await requests_client.get(config("ADVICE_URL", default="https://api.adviceslip.com/advice"))
...
async def apod(request: Request):
    ...
    data = response.json()  # Get JSON response directly

    # Return template response with request and the API data
    return templates.TemplateResponse("apod.html", {"request": request, "serverTime": serverTime, **data})  # Unpack data directly
